...
const char HTML[] =
    ...
    "<head>\n"
    "    <style>\n"
    "        .statsTable { border-collapse: collapse; border-width: 1px; border-style: outset; font-family: 'lucida grande'; font-size: 15px; width: %s; height: %s; }\n"
    "        .cellCommon { border-width: 1px; border-style: solid; border-color: black; padding: 4px; text-align: center; }\n"
    "        .cellHeader { background-color: #12b2c4; }\n"
    "        .cellBlacklisted { background-color: red; }\n"
    "        .cellDisabled { border-color: black; color: #a8a8a8; }\n"
    "        .cellNoLastFail { color: #eee; }\n"
    "        .cellUpstream { }\n"
    "        .cellUpstreamOne { background-color: #3ec7d6; }\n"
    "        .cellUpstreamTwo { background-color: #12a2b3; }\n"
    "        .cellImplicitUpstream { color: #fff; }\n"
    "        .cellSortAsc { }\n"
    "        .cellSortDesc { }\n"
    "        .cellSortTrigger { cursor: pointer; }\n"
    ...
    if (state[0].peer && state[0].status) {
        ...
        if (tp->sec != stat_now && ngx_atomic_cmp_set(ts_ptr, stat_now, tp->sec)) {
            ...
            volatile ngx_uint_t *history_speed_ptr = (ngx_uint_t *)USTATS_CALC_ADDRESS(
                        rrp->peers->peer[rrp->current].shm_start_offset, speed_offset);
