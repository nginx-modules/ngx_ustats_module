...
def read_file(fname):
  ...
  try:
    ...
    print "Unexpected error on parsing persistence data: ", sys.exc_info()[0]
...
def upstream_500(ustats, options):
  ...
  if upstream_persistence is None or upstream_persistence['start'] != start_time:
    if upstream_persistence is None:      
      restart = []
    else:
      restart = upstream_persistence['restart']
      
    if start_timestamp > 0 and (len(restart) == 0 or restart[-1] != start_timestamp):
      restart.append(start_timestamp)

    upstream_persistence = {'start': start_time, 'restart': restart, 'ticks': [[current_tick, count_500]]}
  elif upstream_persistence['ticks'][-1][0] == current_tick:
    upstream_persistence['ticks'][-1][1] = count_500
  else:
    upstream_persistence['ticks'] = [tick for tick in upstream_persistence['ticks'] if tick[0] > ( int( time.time() ) - options.tick * options.tick_num )]
    upstream_persistence['ticks'].append([current_tick, count_500])
    
  restart = upstream_persistence['restart']
  restart = [ts for ts in restart if ts > ( int( time.time() ) - options.tick * options.tick_num )]
  ...
  if len(upstream_persistence['ticks']) < 2:
    ...
    print (upstream_persistence['ticks'][-1][1] - upstream_persistence['ticks'][0][1])
